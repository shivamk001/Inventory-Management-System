...
class SpreadSheet(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.cellframe = tk.Frame(self)#.task_canvas)
        self.cellframe.pack(side='top')

        #self.scrollbarv=tk.Scrollbar(self.tasks_canvas, orient='vertical', command=self.tasks_canvas.yview)
        #self.scrollbarh=tk.Scrollbar(self.tasks_canvas, orient='horizontal', command=self.tasks_canvas.xview)

        #self.tasks_canvas.configure(yscrollcommand=self.scrollbarv.set)
        #self.tasks_canvas.configure(xscrollcommand=self.scrollbarh.set)
...
def spreadSheet():
    root = tk.Tk()
    root.title('I.V.M.S Spreadsheet')
    #root.geometry('240x240')
    root.iconbitmap(r'favicon.ico')
    ss=SpreadSheet(Nrows, Ncols, master=root)
    ss.mainloop()
